...
var List = {
    ...
    Columns: [
        ...
        { name: "ID", width: "10%", classes: "base-table-mobile-hide", sort: "" },
        { name: "Adı", width: "30%", sort: "Name" },
        { name: "Açıklama", width: "40%", classes: "base-table-mobile-hide", },
        { name: "İşlem", width: "10%", sort: "" },
    ],
    ...
    GetData: function () {
        Backend.Post(
            ...
            function (response) {
                if (response.User == 0) {
                    $('#ErrorDiv').html('<h5 class="alert alert-danger" style="text-align:center;width:100%;">Kullanıcı Bulunamadı...</h5>')
                    return;
                }
                List.PrepareEntity(response.User);
                List.AddRows(response.Roles);
                List.SelectRoles(response.UserRoles);
            }
        )
    },
    ...
    PrepareEntity: function (entity) {
        ...
        var htmlContent = ''
            ...
            + '    <div class="form-group row  col-md-6 col-lg-6 col-sm-12 col-xs-12">'
            + '        <label for="editName" class="col-md-4 col-lg-4 col-sm-4 col-xs-4 col-form-label">Adı</label>'
            + '        <div class="col-md-8 col-lg-8 col-sm-8 col-xs-8">'
            + '            <input type="text" class="form-control disabled="disabled" tabkey" value="' + entity.FirstName + '" />'
            + '        </div>'
            + '    </div>'


            + '    <div class="form-group row  col-md-6 col-lg-6 col-sm-12 col-xs-12">'
            + '        <label for="editDescription" class="col-md-4 col-lg-4 col-sm-4 col-xs-4 col-form-label">Email</label>'
            + '        <div class="col-md-8 col-lg-8 col-sm-8 col-xs-8">'
            + '          <input type="text" class="form-control disabled="disabled" tabkey" value="' + entity.Email + '" />'
            + '        </div>'
            + '    </div>'


        $("#EntityForm").html(htmlContent);
    },
    ...
    AddRows: function (rows) {
        ...
        for (var i in rows) {
            row = rows[i];
            List.count++;
            htmlRows += '<tr data-row-id="' + row.ID + '" class="GridRow" onmouseout="this.className=\'GridRow\'" onmouseover="this.className=\'GridRowOver\'">'
                ...
                + '<td style="text-align:center;" ><input type="checkbox" style="height:23px;width:23px;" class="cxbRole" /></td>'
                + '</tr>';
        }

        htmlRows += '<tr><td colspan="100%"><button onclick="List.EditUserRoles()" class="btn btn-primary btn-block"><i class="fas fa-save"></i> Kaydet</button></td></tr>'
        $(htmlRows).appendTo($("#tableRows"));
    },

    SelectRoles: function (roles) {
        $(roles).each(function (i, v) {
            $('#tableRows tr[data-row-id="' + v.ID + '"]').find('.cxbRole').prop('checked', true);
        });
    },
    ...
    EditUserRoles: function () {
        var roleIDs,
        $selectedRows = $("#tableRows  > tr > td:last-child > input:checked").closest("tr");

        roleIDs = $selectedRows.map(function () { return parseInt($(this).data("row-id")); }).get();
        Backend.Post(
            "EditUserRoles",
            {
                userID: List.id,
