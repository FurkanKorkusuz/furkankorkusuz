...
var List = {
    ...
    Columns: [
        ...
        { name: "Blog Sayısı", width: "10%", classes: "base-table-mobile-hide", sort: "BlogCount" },
    ...
    AddRows: function (rows) {
        ...
        for (var i in rows) {
            ...
            htmlRows += '<tr class="GridRow" onmouseout="this.className=\'GridRow\'" onmouseover="this.className=\'GridRowOver\'">'
                ...
                + '<td class="base-table-mobile-hide">' + row.BlogCount + '</td>'
                + '<td style="text-align:center;" >'
                + '<button class="btn btn-outline-info btn-sm" onclick="List.FindForEdit(' + row.ID + ',this)"><i class="fas fa-pen-square"></i> Düzenle</button>&nbsp;'
                + '<button class="btn btn-outline-danger btn-sm" onclick="List.DeleteDialog(' + row.ID + ',this)"><i class="fas fa-trash"></i> Sil</button>'
                + '</td>'
                + '</tr>';
        }
    ...
    DeleteDialog: function (id) {
        var htmlContent = ''
            ...
            + '<div style="margin-top:3px;" class="alert alert-danger" id="DeleteDialogError">Seçtiğiniz kategoride Blog bulunmuyorsa silinecektir.<br>Onaylıyor musunuz?</div>'
